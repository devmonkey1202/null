...
## 11. Why GitHub `main` Looks Stale

현재 최신 작업은 `main`이 아니라 `v2-rebuild`에만 올라갑니다.

따라서 GitHub 저장소 메인 화면에서 `main`을 보고 있으면:
- 폴더 수정 시점이 오래돼 보일 수 있음
- 최신 v2 editor 커밋이 보이지 않을 수 있음

이것은 누락이 아니라 의도된 운영 방식입니다.

확인 규칙:
- 최신 진행 상황 확인: `v2-rebuild`
- 운영 기준 확인: `main`

`main` 반영은 README와 06 문서의 merge gate를 통과한 뒤에만 진행합니다.
